...
void InitApp()
{
	...
	g_HUD.AddButton(IDC_OPENBUTTON, L"Open file ...", 35, iY += 25, 125, 22);
...
bool CALLBACK ModifyDeviceSettings( DXUTDeviceSettings* pDeviceSettings, void* pUserContext )
{
    ...
    // then switch to SWVP.
    if( (g_sCaps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT ) == 0 ||
		g_sCaps.VertexShaderVersion < D3DVS_VERSION( 1, 1 ) )
    {
    ...
    *pbNoFurtherProcessing = g_SampleUI.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	if (WM_SIZE == uMsg)
	{
		if (SIZE_MINIMIZED != wParam)
		{
			...
			g_Model.SetWindowRect(&rcCurrentClient);
		}
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		case IDC_OPENBUTTON:
		{
			...
			ofn.lpstrFilter = "FBX Files (*.fbx)\0*.fbx\0\0";
